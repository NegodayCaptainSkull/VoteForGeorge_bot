...
(function (UserState) {
    UserState["NONE"] = "NONE";
    UserState["AWAITING_SUGGESTION"] = "AWAITING_SUGGESTION";
    UserState["AWAITING_JOIN_REQUEST"] = "AWAITING_JOIN_REQUEST";
})(UserState || (UserState = {}));
...
bot.command('start', async (ctx) => {
    await ctx.reply('Добро пожаловать! Выберите опцию ниже:', {
...
bot.hears('Мини-игра', async (ctx) => {
    await ctx.reply('Добро пожаловать в SchoolCoin кликер! Нажмите кнопку ниже, чтобы открыть игру:', {
        reply_markup: new InlineKeyboard().webApp('Открыть игру', 'https://vote-for-george.vercel.app/' // Ссылка на ваше Web App
        ),
    });
});
