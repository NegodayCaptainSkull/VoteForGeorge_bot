...
**1. Интерактив: В этом году ко Дню дублера у нас появится новая игра School Coin, где каждый сможет заработать виртуальные коины. Это не просто игра — это шанс проявить себя и побороться за крутые призы, которые получат самые активные участники!** 🎮💰

**2. Командная работа: Я открыт для ваших идей и всегда готов поддержать, обсудить и реализовать самые крутые предложения от учеников.**

**3. Яркие идеи и вдохновение: Я хочу, чтобы День дублера стал незабываемым событием, полным интересных активностей, креативных идей и приятных сюрпризов. Каждый ученик почувствует атмосферу праздника и станет частью чего-то особенного!** ✨

🎯 **Что делать?**

**1. Голосуйте за меня, Гошу Мкртчяна, на День дублера!**
**2. Зарабатывайте коины в игре и готовьтесь к наградам!** (Раздача призов будет только в случае победы кандидата)

💡 
> Голосуй за Георгия Мкртчяна — и сделай День дублера не просто днем, а настоящим праздником! 
💪
`;
...
bot.hears('Сделать предложение 💬', async (ctx) => {
  ...
  await ctx.reply('💡 У тебя есть классная идея, как сделать День дублера ещё лучше? Напиши её прямо сюда, и я обязательно её рассмотрю! Вместе мы сделаем этот день ярким и запоминающимся. 😉 Если у тебя есть замечания по моей работе или работе бота можешь также написать сюда. 😁', {
    reply_markup: new Keyboard().text('Отмена ❌').row()
  });
});
...
// Обработчик кнопки "Отправить музыку 🎵"
...
bot.hears('Отмена ❌', async (ctx) => {
  ...
  await ctx.reply('Ожидание отменено. Выберите опцию из меню:', {
    reply_markup: new Keyboard()
      .text('О кандидате 👤')
      .row()
      .text('Сделать предложение 💬')
